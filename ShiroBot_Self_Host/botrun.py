...
description = "Self Host Version Of the Shiro Bot"
...
@bot.event
async def loop_task():
	presence = ["{}help | Homo Erectus".format(prefix), "github.com/StrawHatHacker/Shiro"]
...
async def on_member_join(member):
	if member_join_message == True:
		try:
			embed=discord.Embed(description = "**{}** to **{}**!!!".format(member.name, member.server), color = color_purple)
			embed.set_author(name = "Welcome", icon_url = member.avatar_url)
			await bot.send_message(bot.get_channel(member_join_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send a *join message*, do I have the right permissions and do i have the correct id of the channel?")

	if member_join_log == True:
		try:
			embed=discord.Embed(description = "{}#{}\nID - {}".format(member.name, member.discriminator, member.id) , color = color_green)
			embed.set_author(name = "--JOINED--", icon_url = member.avatar_url)
			await bot.send_message(bot.get_channel(member_log_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send a *join log message*, do I have the right permissions and do i have the correct id of the channel?")

@bot.event
async def on_member_remove(member):
	if member_leave_log == True:
		try:
			embed=discord.Embed(description = "{}#{}\nID - {}".format(member.name, member.discriminator, member.id) , color = color_red)
			embed.set_author(name = "**LEFT**", icon_url = member.avatar_url)
			await bot.send_message(bot.get_channel(member_log_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send a *leave log message*, do I have the right permissions and do i have the correct id of the channel?")		

@bot.event
async def on_message_delete(message):
	if message.author == bot.user:
		return
	if message_delete_log == True:
		try:
			embed=discord.Embed(description = "Author: {}#{}\nChannel: #{}\n\nContent:\n{}\n\nMessage ID:\n{}".format(message.author.name, message.author.discriminator, message.channel.name, message.system_content, message.id), color = color_red)
			embed.set_author(name = "DELETED", icon_url = message.author.avatar_url)
			await bot.send_message(bot.get_channel(message_log_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send a *deleted message log*, do I have the right permissions and do i have the correct channel id?")
...
async def on_message_edit(before, after):
	...
	if before.content == after.content:
		return
	if message_edit_log == True:
		try:
			embed=discord.Embed(description = "Author: {}#{}\nChannel: #{}\n\nBefore:\n{}\n\nAfter:\n{}\n\nMessage ID:\n{}".format(before.author.name, before.author.discriminator, before.channel.name, before.content, after.content, after.id), color = color_green)
			embed.set_author(name = "EDITED", icon_url = before.author.avatar_url)
			await bot.send_message(bot.get_channel(message_log_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send an *edited message log*, do I have the right permissions and do i have the correct channel id?")

@bot.event
async def on_member_unban(server, member):
	if member_unban_log == True:
		try:
			embed=discord.Embed(description = "User: {}#{}\nID - {}".format(member.name, member.discriminator, member.id), color = color_yellow)
			embed.set_author(name = "UNBANNED", icon_url = member.avatar_url)
			await bot.send_message(bot.get_channel(member_bankick_channel_id), embed=embed)
		except:
			await bot.send_message(bot.get_channel(bot_test_channel_id), "I could not send an *member unban log*, do I have the right permissions and do i have the correct channel id?")
...
async def test(ctx):
	try:
		await bot.send_message(bot.get_channel(bot_test_channel_id), "Yay I can send a normal message here, dont forget to give me embed permissions")
	except:
		await bot.send_message(ctx.message.author, "I can't send a message to {}, give me permissions and check if the id is correct".format(ctx.message.channel.name))
...
async def kick(ctx, member, *, reason="None specified"):
	...
	try:
		...
		await bot.say(embed=embed)
		if member_ban_log == True:
			try:
				embed1=discord.Embed(color = color_green)
				embed1.add_field(name = "Member Kicked", value = "{}#{}\nID - {}".format(member.name, member.discriminator, member.id))
				embed1.add_field(name = "Responsible Moderator", value = "{}".format(ctx.message.author.name))
				embed1.add_field(name = "Reason", value = "{}".format(reason))
				await bot.send_message(bot.get_channel(member_bankick_channel_id), embed=embed1)
			except:
				await bot.send_message(bot.get_channel(bot_test_channel_id),  "I could not send an *member kick log*, do I have the right permissions and do i have the correct channel id?")
...
async def ban(ctx, member, *, reason="None specified"):
	...
	try:
		...
		if member_ban_log == True:
			try:
				embed1=discord.Embed(color = color_red)
				embed1.add_field(name = "Member Banned", value = "{}#{}\nID - {}".format(member.name, member.discriminator, member.id))
				embed1.add_field(name = "Responsible Moderator", value = "{}".format(ctx.message.author.name))
				embed1.add_field(name = "Reason", value = "{}".format(reason))
				await bot.send_message(bot.get_channel(member_bankick_channel_id), embed=embed1)
			except:
				await bot.send_message(bot.get_channel(bot_test_channel_id),  "I could not send an *member ban log*, do I have the right permissions and do i have the correct channel id?")
	except Exception as e:
		if 'Privilege is too low' in str(e):
...
async def sban(ctx, member, *, reason="None specified"):
	...
	try:
		...
		if member_ban_log == True:
			try:
				embed1=discord.Embed(color = 0x3333cc)
				embed1.add_field(name = "Member Banned", value = "{}#{}\nID - {}".format(member.name, member.discriminator, member.id))
				embed1.add_field(name = "Responsible Moderator", value = "{}".format(ctx.message.author.name))
				embed1.add_field(name = "Reason", value = "{}".format(reason))
				await bot.send_message(bot.get_channel(member_bankick_channel_id), embed=embed1)
			except:
				await bot.send_message(bot.get_channel(bot_test_channel_id),  "I could not send an *member sban log*, do I have the right permissions and do i have the correct channel id?")
	except Exception as e:
		if 'Privilege is too low' in str(e):
...
async def help(ctx):
	...
	embed.add_field(name = "Owner Commands", value = "`setgame`, `serverinfo`, `botservers`")
	embed.add_field(name = "Admin/Mod Commands", value = "`ban`, `sban`, `kick`, `warn`, `mute`, `unmmute`, `ping`, `prune`, `bans`")
	embed.add_field(name = "Reactions", value = "`cute`, `kiss`, `hug`, `pat`, `slap`, `blush`, `lick`, `mad`, `scared`, `tired`, `cry`, `nimu`, `tickle`, `run`, `bite`, `plot`, `nervous`")
...
async def setgame(ctx, *, game:str):
	if ctx.message.author.id in owner_ids:
		...
		x = await bot.say("You aren't the bot owner!")
		await asyncio.sleep(5)
		await bot.delete_message(x)
